...
void uart_init (void) {
  ...
  uart_enable_rx_irq();  // do not enable tx irq
...
/* if the UART is idle, and a character is waiting in the transmit buffer, send
it. caller must hold uart_tx_lock. called from both the top- and bottom-half.

fxl: So the code below spin waits until all chars go out. 

ideally (as in xv6) this func starts uart tx and returns, w/o waiting for the
chars to go out "on wire". It's more efficeint. But I couldn't get that 100%
working, and found it tricky to debug. 
*/
void uartstart() {
    while (1) {
