...
// lab3 - quite unused fields below. kept so that we dont have to tune macros at the end of this files
struct task_struct {
    // private to the task, no task->lock needed
    struct cpu_context cpu_context; // MUST COME FIRST. register values.
    struct file *ofile[NOFILE];     // Open files   - unused for lab3
    struct inode *cwd;              // Current directory - unused for lab3
