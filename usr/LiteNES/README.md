LiteNES0 - NES with executable image in-kernel

design principles:
- a user task that can run with UI
- minimum kernel dep (no exec(), no fs, no /proc/fs etc)

details: 
- run as user task: EL0, has own VM
- executable to be linked to the kernel image
- no fs; no rom load, hence can only run Mario which is built in
- only gfx - via mapped fb (kernel has to do so), no game input, so can only watch
- kernel passes fb config as args to main()
- syscall needed: 
    write()  // for print msgs
    sys_sleep() // for periodic rendering
    brk() // for malloc 
    exit() // for cleanup (can be removed)
